...
void key_move(){
    if(front == 0 && mode1_flag == 0){//전진_모드
        mode1_flag = 1; 
        mode = 1;
    }
    if(back == 0 && mode2_flag == 0){//후진모드
        mode2_flag = 1;
        mode = 2;
    }      
    if(left == 0 && mode3_flag == 0){//좌회전모드
        mode3_flag = 1;
        mode = 3;
    }
    if(right == 0 && mode4_flag == 0){//우회전모드
        mode4_flag = 1;
        mode = 4;
    }  
    if(fire_key == 0 && mode5_flag == 0){//발사모드
        mode5_flag = 1;
        mode = 5;
    }        
    if(drone_left_key == 0 && mode6_flag == 0){//드론_카메라_좌회전
        mode6_flag = 1;
        mode = 6;
    }
    if(drone_right_key == 0 && mode7_flag == 0){//드론_카메라_우회전
        mode7_flag = 1;
        mode = 7;
    }    
    if(safety_key == 0 && mode8_flag == 0){ //안전모드
        mode8_flag = 1;
        mode = 8;
    }
    if(semiauto_key == 0 && mode9_flag == 0){//단발모드
        mode9_flag = 1;
        mode = 9;
    }           
    if(brust_key == 0 && mode10_flag == 0){  //점사모드
        mode10_flag = 1;
        mode = 10;
    }
    if(auto_key == 0 && mode11_flag == 0){   //연사모드
        mode11_flag = 1;
        mode = 11;
    }    
}
...
/****IR_led_신호_출력_****/
/********IR_LED는 TV리모컨을 뜯어서 했기 때문에 1을 주면 저절로 PWM이 생깁니다.*******/
void ir_sig(){
    //전진
    if(mode == 1){
        sig_point++;
